...
## 12. Safety / failsafe (`Siemens.Engineering.Safety`, reflection-probed 2026-08-28, runtime-verified 2026-09-01, corrected 2026-09-02)

The namespace is documented in the TIA Openness manual §5.27 ("F-related Openness"; the V17
assembly matches). Runtime-verified against `PEI_SinoARP_Master_V4.1.3` (CPU 1515F-2 PN,
6ES7 515-2FM01-0AB0/V2.9, 1152 blocks of which 55 failsafe, safety system V2.4) in both
`WithoutUserInterface` and UI mode with `scripts/Probe-SafetySignature.ps1`.

- **Safety device detection — anchor is the DeviceItem, NOT the PlcSoftware (verified):**
  `plcSoftware.GetService<SafetyAdministration>()` always returns **null**, even on a genuine
  F-CPU. The same `GetService` call on the PLC's **DeviceItem** (walk `plc.Parent` up to the
  first `DeviceItem`) returns the service. The detection rule: service present → safety device;
  any exception → read-failed, never "ordinary PLC". (On this F-CPU: `SafetyAdministration` and
  `SafetyPrintout` present on the DeviceItem.)
- **`SafetySignatureProvider` is anchored on the F-block (PlcBlock), per the manual (§5.27.4) —
  verified 2026-09-02:** `block.GetService<SafetySignatureProvider>()` returns the provider on
  every F-block and **null on non-F-blocks** (the semantic F-block detector — also used by
  `FailSafeBlocks.IsFailSafe`). On the DeviceItem anchor the provider is **always null**; the
  2026-09-01 "license-gated provider" conclusion was an artifact of that wrong anchor. Per-block
  `Signatures.Find(SafetySignatureType.BlockOfflineSignature)` (`UInt64`) reads without safety
  login; a value of **0 means "signature missing or invalidated by a recent change"** (manual).
  `TiaV17Adapter.ReadSafety` folds all per-block values (sorted `path|HEX` lines, SHA-256,
  spaced hex pairs) into the PLC-level `FSignature`; `no-signature` now means no block exposed a
  signature at all (e.g. safety program never compiled). Whether the per-block provider is
  readable on a machine *without* a STEP 7 Safety license is untested — the remaining probe item.
- **`SafetyAdministration`** reads work **without** a license and without safety login:
  ...
  mainSafetyBlock `000_Main_Safety`, FOB `FOB_SAFETY`, maxCycleTime 200 ms). License is required
  only for its *mutating* actions (manual §5.27.5: "Reading values is possible without a
  license").
- **`FingerprintProvider.GetFingerprints()` works on F-blocks (verified):** all 55 F-blocks
  ...
  license. Since F-block export is refused, this remains a viable per-block change signal
  alongside the folded signature.
...
- Wired up: `get_plc_checksums` returns `IsSafetyDevice`, `FSignatureReadState`
  ...
  (`TiaV17Adapter.ReadSafety`); the same evidence is on `get_context_status` / `compare_context`.
  The workbench compare (`WorkbenchConsistencyService.CompareAsync`) checks the live signature
  against master's revision.json on every compare: a changed signature, or a lost safety surface
  on a device that had a baseline signature, makes the result `Different` + `SafetyChanged`; a
  failed required read or degraded evidence (baseline signature present, live read empty) makes
  it `Unavailable` — never silently consistent. `revision.json` additionally records
  `safety.readState`; per-device safety fields ride along in the `tia-state/{sha}` commit-state
  tags (schema "1.1").
