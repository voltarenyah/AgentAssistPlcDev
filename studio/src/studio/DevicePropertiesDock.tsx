    ...
    <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
      {visible.map(([label, value]) => (
        <div key={label} className="px-2 py-1.5">
          ...
          <div className={`mt-0.5 break-words text-xs leading-4 ${mono ? 'font-mono' : ''}`}>{value}</div>
        </div>
      ))}
    </div>
  )
  ...
  return (
    ...
    >
      <div className="flex h-12 items-center gap-2 border-b px-3" style={{ borderColor: 'var(--border)' }}>
        ...
        <h2 className="text-sm font-semibold">Device properties</h2>
      </div>
        ...
        {!meta && !info ? (
          ...
          <div className="space-y-2">
            {meta && deviceRows.some(([, value]) => value) && (
              <section className="rounded-md border bg-background" style={{ borderColor: 'var(--border)' }}>
                <div className="border-b px-2 py-1.5" style={{ borderColor: 'var(--border)' }}>
                  ...
                  <div className="mt-0.5 text-xs text-muted-foreground">Captured at last export</div>
                </div>
            ...
            {meta && projectRows.some(([, value]) => value) && (
              <section className="rounded-md border bg-background" style={{ borderColor: 'var(--border)' }}>
                <div className="border-b px-2 py-1.5" style={{ borderColor: 'var(--border)' }}>
                  <div className="text-sm font-medium">TIA project</div>
                  <div className="mt-0.5 text-xs text-muted-foreground">Captured at last export</div>
                </div>
                <PropertyRows rows={projectRows} />
                {meta.projectComment && (
                  <div className="border-t px-2 py-1.5" style={{ borderColor: 'var(--border)' }}>
                    <div className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Project comment</div>
                    <div className="mt-0.5 whitespace-pre-wrap text-xs leading-4">{meta.projectComment}</div>
                  </div>
                )}
              </section>
            )}
