...
type Props = { workbenchId: string; worktreeId: string; branch: string }

export default function VersionControlCompare({ workbenchId, worktreeId, branch }: Props) {
  ...
  const accept = async () => {
    ...
    setBusy(true); setError(null)
    try { await api.acceptTiaSynchronization(workbenchId, comparison.comparisonId, [...selected]); await compare() }
  ...
  return (
    <section className="flex h-full min-h-0 flex-col" aria-label="Compare PLC source with TIA">
      <div className="flex shrink-0 items-center justify-between border-b px-3 py-2" style={{ borderColor: 'var(--border)' }}>
        ...
        <div className="flex gap-1"><button type="button" className="rounded bg-accent px-2 py-1 text-[9px]" onClick={() => void compare()} disabled={busy}>{busy ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Compare now'}</button>{branch && branch.toLowerCase() !== 'master' && <button type="button" className="rounded bg-chart-4 px-2 py-1 text-[9px] text-white" onClick={() => void prepare()} disabled={busy}>Prepare feature import</button>}</div>
      </div>
      {error && <div className="px-3 py-2 text-[9px] text-destructive">{error}</div>}
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        {!comparison ? <div className="py-8 text-center text-[9px] text-muted-foreground">Compare the connected TIA project with master to see PLC-object differences.</div> : comparison.fastGatePassed && comparison.differences.length === 0 ? (
          <div className="rounded border border-emerald-500/30 bg-emerald-500/5 p-3 text-[10px]"><div className="flex items-center gap-2 font-medium text-emerald-600"><CheckCircle2 className="h-4 w-4" />All device checksums match</div><div className="mt-2 text-[8px] text-muted-foreground">No full object scan was required.</div>{Object.entries(comparison.liveChecksums).map(([device, checksum]) => <div key={device} className="mt-1 font-mono text-[8px]">{device}: {checksum ?? 'Unavailable'}</div>)}</div>
        ) : (
          <div className="space-y-2"><div className="rounded border border-amber-500/30 bg-amber-500/5 p-2 text-[9px] text-amber-700">{comparison.state === 'Unavailable' ? 'TIA checksum unavailable.' : 'TIA differs from master; select supported changes to accept.'}</div>{comparison.differences.map(diff => { const path = diff.relativePath; const disabled = !diff.supported || diff.kind === 'Deleted' || !path; return <label key={`${diff.deviceId}:${path}:${diff.identity}`} className={`flex items-start gap-2 rounded border p-2 ${disabled ? 'opacity-60' : 'hover:bg-accent/40'}`} style={{ borderColor: 'var(--border)' }}><input type="checkbox" disabled={disabled} checked={selected.has(path)} onChange={event => setSelected(previous => { const next = new Set(previous); if (event.target.checked) next.add(path); else next.delete(path); return next })} /><span className="min-w-0 flex-1"><span className="block text-[9px] font-medium">{diff.plcName} · {diff.identity || diff.relativePath}</span><span className="block truncate font-mono text-[8px] text-muted-foreground">{path || 'Source coverage unavailable'}</span><span className="block text-[8px] text-muted-foreground">{diff.supported ? diff.kind : 'Source coverage unavailable'}</span></span>{!diff.supported && <ShieldAlert className="h-3 w-3 text-amber-500" />}</label>})}{selected.size > 0 && <button type="button" className="w-full rounded bg-chart-2 px-2 py-1.5 text-[9px] text-white" onClick={() => void accept()} disabled={busy}>Accept selected into master ({selected.size})</button>}</div>
        )}
      </div>
