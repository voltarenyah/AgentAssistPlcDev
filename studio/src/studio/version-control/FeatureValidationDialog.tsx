...
export default function FeatureValidationDialog({ workbenchId, featureWorktreeId, plan, onClose }: Props) {
  ...
  return <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/30 p-4">
    <section className="w-full max-w-lg rounded-lg border bg-card shadow-xl" style={{ borderColor: 'var(--border)' }} aria-label="Feature validation">
      ...
      <footer className="flex justify-end gap-2 border-t px-3 py-2" style={{ borderColor: 'var(--border)' }}>{!session && <button type="button" className="rounded bg-chart-4 px-3 py-1.5 text-[9px] text-white" disabled={busy || selected.size === 0} onClick={importSelected}>{busy ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Import selected'}</button>}{session && !validation && <button type="button" className="rounded bg-chart-4 px-3 py-1.5 text-[9px] text-white" disabled={busy || !machineValidated} onClick={validate}>{busy ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Compile all devices'}</button>}{isReady && <button type="button" className="rounded bg-emerald-600 px-3 py-1.5 text-[9px] text-white" disabled={busy || !machineValidated} onClick={merge}>{busy ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Merge validated feature'}</button>}</footer>
    </section>
  </div>
}
