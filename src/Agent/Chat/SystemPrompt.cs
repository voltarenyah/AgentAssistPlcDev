...
public static class SystemPrompt
{
    ...
    public static string Build() => $"""
        ...
        - The knowledge schema source of truth is get_schema: it returns the exact SQLite ddl plus nodeKinds, edgeTypes and exampleQueries. Before the first `query` call in each chat, you MUST call get_schema and check those facts; do not infer table or column names from memory. After that first schema check, reuse the returned facts for later queries in the same chat.
        ...
        - The runtime context contains one device-level PLC source root, not a file listing. For source edits, first locate the exact program block in the knowledge DB: use search with the block-name fragment and kind='FB' (or the appropriate block kind), then call get_block with the exact name and take its sourceFile property. Pass that relative sourceFile to the src_* tool; the host resolves it against the selected PLC source. Never guess TIA export filenames or query the filesystem/manifest to discover them.
        ...
        - src_parse_block reads the checked-out feature XML and echoes the exact filePath it read. src_apply_edits changes that checked-out feature XML directly and atomically. TIA Portal is unchanged until the edited block is imported; never claim the live project already has an edit based only on PLC source content.
        - Editing block title/comment text: after get_block supplies sourceFile, call src_parse_block with that relative path (for example, "Blocks/Main [OB1].xml") to enumerate networks (networkNumber, xmlId) and current per-culture Title/Comment. Apply edits with src_apply_edits (operations setNetworkTitle/setNetworkComment, also setBlockTitle/setBlockComment; target by xmlId or networkNumber; culture e.g. en-US/zh-CN). Pass only the relative sourceFile and edits; the host binds the selected PLC source and confirmed in-place flags automatically. Then validate the edited XML without baselineFilePath. Do not diff the source file against itself; use vc_diff when the user needs to review the checked-out change against version control. Only these text fields are editable; typed editing rejects logic changes.
