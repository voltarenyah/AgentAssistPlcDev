...
public sealed class DeviceReconciler
{
    ...
    private static void AddHardwareArtifacts(
        string root,
        IDictionary<string, ManifestComponent> controlled,
        bool requireReferencedFiles,
        string metadataPath)
    {
        var hardwareManifestPath = Path.Combine(root, "Hardware", "manifest.json");
        if (!File.Exists(hardwareManifestPath))
        {
            return;
        }

        JsonDocument hardwareDocument;
        try
        {
            using var stream = File.OpenRead(hardwareManifestPath);
            hardwareDocument = JsonDocument.Parse(stream);
        }
        catch (Exception exception) when (
            exception is JsonException
            or IOException
            or UnauthorizedAccessException)
        {
            throw new ReconciliationException(
                ManifestInvalidCode,
                $"The hardware manifest '{hardwareManifestPath}' could not be read.",
                exception);
        }

        using (hardwareDocument)
        {
            var hardwareRoot = hardwareDocument.RootElement;
            if (hardwareRoot.ValueKind != JsonValueKind.Object)
            {
                throw new ReconciliationException(
                    ManifestInvalidCode,
                    $"The hardware manifest '{hardwareManifestPath}' does not have the required shape.");
            }
            ...
            AddHardwarePath(
                HardwarePath(hardwareRoot, "projectAmlFile"),
            ...
            AddHardwarePath(
                HardwarePath(hardwareRoot, "projectLogFile"),
            ...
            if (hardwareRoot.TryGetProperty("devices", out var devices))
            {
                if (devices.ValueKind != JsonValueKind.Array)
                {
                    throw new ReconciliationException(
                        ManifestInvalidCode,
                        $"The hardware manifest '{hardwareManifestPath}' has an invalid devices value.");
                }
                ...
                foreach (var device in devices.EnumerateArray())
                {
                    if (device.ValueKind != JsonValueKind.Object)
                    {
                        throw new ReconciliationException(
                            ManifestInvalidCode,
                            $"The hardware manifest '{hardwareManifestPath}' contains a malformed device.");
                    }

                    AddHardwarePath(
                        HardwarePath(device, "amlFile"),
                        $"hardware:device-{deviceIndex}-aml",
                        root,
                        controlled,
                        requireReferencedFiles,
                        hardwareManifestPath);
                    AddHardwarePath(
                        HardwarePath(device, "logFile"),
                        $"hardware:device-{deviceIndex}-log",
                        root,
                        controlled,
                        requireReferencedFiles,
                        hardwareManifestPath,
    ...
    private static string? HardwarePath(JsonElement element, string propertyName)
    {
            ...
            || string.IsNullOrWhiteSpace(value.GetString()))
        {
            throw new ReconciliationException(
                ManifestInvalidCode,
                $"Hardware manifest property '{propertyName}' must be a non-empty string or null.");
        }

        return Path.Combine("Hardware", value.GetString()!.Replace('/', Path.DirectorySeparatorChar));
    }

    private static void AddHardwarePath(
        string? relativePath,
        string identity,
        string root,
        IDictionary<string, ManifestComponent> controlled,
        ...
        bool required = true)
    {
        if (!required && relativePath is null)
        {
            return;
        }
        ...
        if (relativePath is null)
        {
            throw new ReconciliationException(
                ManifestInvalidCode,
                $"The hardware manifest '{sourceManifestPath}' is missing a required artifact path.");
        }

        var normalized = NormalizeRelativePath(root, relativePath);
        if (!controlled.TryAdd(
                normalized,
                new ManifestComponent(normalized, identity, null)))
        {
            throw new ReconciliationException(
                ManifestInvalidCode,
                $"The hardware manifest '{sourceManifestPath}' lists '{normalized}' more than once.");
        }

        if (requireReferencedFiles
            && !File.Exists(ResolveControlledPath(root, normalized)))
        {
            throw new ReconciliationException(
                ManifestInvalidCode,
                $"The hardware manifest references missing file '{normalized}'.");
        }
    }
