...
| **Amended: workbench creation asks for nothing** | Measured after the change, `vc_add_worktree` and `create_project` are tier **write**, not **destructive** — the catalog is 43 read / 23 write / 7 destructive / 20 unknown. An earlier version of this ADR asserted they were destructive; that was wrong. The panel therefore raises the approval card only for the destructive tools (`import_block`, `vc_restore` and five others) and **never for workbench creation**, which executes on request. The user accepted this, rather than reclassifying those two tools or adding a second panel-level approval path. So (b) means "the approval card is preserved and wired", not "every workbench creation is confirmed". Reclassifying is a sandbox security-model change and would gate the device chat too. |
| **Known gap in the same area** | `vc_remove_worktree` is tier **unknown**, so the sandbox fails closed on it and neither the panel nor the agent can undo a worktree it created; it had to be removed with `git worktree remove` directly. 20 tools are in the same unknown tier. |
